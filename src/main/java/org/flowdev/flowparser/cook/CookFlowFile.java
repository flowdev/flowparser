...
public class CookFlowFile<T> extends Filter<T, EmptyConfig> {
	...
	private RawConnectionPart findInConnectionPart(String portName, RawFlow rflow) {
		...
		throw new RuntimeException("Unable to find input port '" + portName + "in raw flow: " + PrettyPrinter.prettyPrint(rflow));
	}
	...
	private RawConnectionPart findOutConnectionPart(String portName, RawFlow rflow) {
		for (RawConnectionChain connChain : rflow.connections) {
			int last = connChain.parts.size() - 1;
			RawConnectionPart connPart = connChain.parts.get(last);
			if (isParentPart(connPart) && portName.equals(connPart.outPort.name)) {
				return connChain.parts.get(last - 1);
			}
		}
		throw new RuntimeException("Unable to find output port '" + portName + "in raw flow: " + PrettyPrinter.prettyPrint(rflow));
	}
	...
	private void cookConnections(RawFlow rflow, Flow flow) {
		...
		for (RawConnectionChain connChain : rflow.connections) {
			...
			for (int i = 0; i < last; i++) {
				addConnection(connChain.parts.get(i), connChain.parts.get(i + 1), flow.connections);
			}
		}
	}
	private void addConnection(RawConnectionPart from, RawConnectionPart to, List<Connection> conns) {
...
//		op.creators = cookOpCreators(rawOp.creators);
	...
	private boolean isParentPart(RawConnectionPart part) {
		return part.inPort == part.outPort;
	}
